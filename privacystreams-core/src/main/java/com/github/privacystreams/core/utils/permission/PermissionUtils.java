...
public class PermissionUtils extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (getIntent() != null && getIntent().getExtras() != null && getIntent().getSerializableExtra(PERMISSIONS_KEY) != null) {
            ...
            ActivityCompat.requestPermissions(PermissionUtils.this, requestedPermissions.toArray(new String[requestedPermissions.size()]), PERMISSION_REQUEST_CODE);
        } else {
        ...
        if (requestCode == PERMISSION_REQUEST_CODE) {
            ...
            if (notGranted > 0) {
                if (this.callbackEnabled) {
                    PermissionUtils.onPermissionRequestCallback.apply(null, false);
                }
                Intent result = new Intent();
                ...
                if (this.callbackEnabled) {
                    PermissionUtils.onPermissionRequestCallback.apply(null, true);
                }
                Intent result = new Intent();
                setResult(Activity.RESULT_OK, result);
                finish();
    ...
    public static boolean requestPermissions(Context context, String[] requiredPermissions) {
        ...
        if (!permissionsGranted) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                ...
                Intent permissionRequest = new Intent(context, PermissionUtils.class);
                permissionRequest.putExtra(PermissionUtils.CALLBACK_ENABLED_KEY, false);
                permissionRequest.putExtra(PermissionUtils.PERMISSIONS_KEY, requiredPermissions);
    ...
    public static void requestPermissionsAndCallback(Context context, String[] requiredPermissions, Callback<Boolean> callback) {
        ...
        if (!permissionsGranted) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                // For Android version M and above, there is chance to request permission at runtime
                PermissionUtils.onPermissionRequestCallback = callback;

                Intent permissionRequest = new Intent(context, PermissionUtils.class);
                permissionRequest.putExtra(PermissionUtils.CALLBACK_ENABLED_KEY, true);
                permissionRequest.putExtra(PermissionUtils.PERMISSIONS_KEY, requiredPermissions);
                permissionRequest.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                context.startActivity(permissionRequest);
            }
