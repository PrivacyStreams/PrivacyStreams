...
//                      provide phone numbers as different types
                        switch(phoneType){
                            case Phone.TYPE_MOBILE:
                                ...
                                if(!phones.containsKey(Contact.MOBILE_PHONE)) phones
                                        .put(Contact.MOBILE_PHONE, mobileList);
                                break;
                            case Phone.TYPE_HOME:
                                homeList.add(CommunicationUtils.normalizePhoneNumber(number));
                                if(!phones.containsKey(Contact.HOME_PHONE)) phones
                                        .put(Contact.HOME_PHONE, homeList);
                                break;
                            case Phone.TYPE_WORK:
                                workList.add(CommunicationUtils.normalizePhoneNumber(number));
                                if(!phones.containsKey(Contact.WORK_PHONE)) phones
                                        .put(Contact.WORK_PHONE, workList);
                                break;
                            case Phone.TYPE_FAX_HOME:
                                homeFaxList.add(CommunicationUtils.normalizePhoneNumber(number));
                                if(!phones.containsKey(Contact.HOME_FAX)) phones
                                        .put(Contact.HOME_FAX, homeFaxList);
                                break;
                            case Phone.TYPE_PAGER:
                                pagerList.add(CommunicationUtils.normalizePhoneNumber(number));
                                if(!phones.containsKey(Contact.PAGER)) phones
                                        .put(Contact.PAGER, pagerList);
                                break;
                            case Phone.TYPE_CALLBACK:
                                callbackList.add(CommunicationUtils.normalizePhoneNumber(number));
                                if(!phones.containsKey(Contact.MOBILE_PHONE)) phones
                                        .put(Contact.MOBILE_PHONE, callbackList);
                                break;
                            case Phone.TYPE_CAR:
                                carList.add(CommunicationUtils.normalizePhoneNumber(number));
                                if(!phones.containsKey(Contact.CAR)) phones
                                        .put(Contact.CAR, carList);
                                break;
                            case Phone.TYPE_ASSISTANT:
                                assistantList.add(CommunicationUtils.normalizePhoneNumber(number));
                                if(!phones.containsKey(Contact.ASSISTANT)) phones
                                        .put(Contact.ASSISTANT, assistantList);
                                break;
                            case Phone.TYPE_COMPANY_MAIN:
                                companyMainList.add(CommunicationUtils.normalizePhoneNumber(number));
                                if(!phones.containsKey(Contact.COMPANY_MAIN)) phones
                                        .put(Contact.COMPANY_MAIN, companyMainList);
                                break;
                            case Phone.TYPE_FAX_WORK:
                                workFaxList.add(CommunicationUtils.normalizePhoneNumber(number));
                                if(!phones.containsKey(Contact.WORK_FAX)) phones
                                        .put(Contact.WORK_FAX, workFaxList);
                                break;
                            case Phone.TYPE_OTHER:
                                otherList.add(CommunicationUtils.normalizePhoneNumber(number));
                                if(!phones.containsKey(Contact.OTHER_PHONE)) phones
                                        .put(Contact.OTHER_PHONE, otherList);
                                break;
                            case Phone.TYPE_OTHER_FAX:
                                otherFaxList.add(CommunicationUtils.normalizePhoneNumber(number));
                                if(!phones.containsKey(Contact.OTHER_FAX)) phones
                                        .put(Contact.OTHER_FAX, otherFaxList);
                                break;
                            case Phone.TYPE_ISDN:
                                ISDNList.add(CommunicationUtils.normalizePhoneNumber(number));
                                if(!phones.containsKey(Contact.ISDN)) phones
                                        .put(Contact.ISDN, ISDNList);
                                break;
                            case Phone.TYPE_MAIN:
                                mainList.add(CommunicationUtils.normalizePhoneNumber(number));
                                if(!phones.containsKey(Contact.MAIN)) phones
                                        .put(Contact.MAIN, mainList);
                                break;
                            case Phone.TYPE_MMS:
                                MMSList.add(CommunicationUtils.normalizePhoneNumber(number));
                                if(!phones.containsKey(Contact.MMS)) phones
                                        .put(Contact.MMS, MMSList);
                                break;
                            case Phone.TYPE_RADIO:
                                radioList.add(CommunicationUtils.normalizePhoneNumber(number));
                                if(!phones.containsKey(Contact.RADIO)) phones
                                        .put(Contact.RADIO, radioList);
                                break;
                            case Phone.TYPE_TELEX:
                                telexList.add(CommunicationUtils.normalizePhoneNumber(number));
                                if(!phones.containsKey(Contact.TELEX)) phones
                                        .put(Contact.TELEX, telexList);
                                break;
                            case Phone.TYPE_TTY_TDD:
                                TTY_TDDList.add(CommunicationUtils.normalizePhoneNumber(number));
                                if(!phones.containsKey(Contact.TTY_TDD)) phones
                                        .put(Contact.TTY_TDD, TTY_TDDList);
                                break;
                            case Phone.TYPE_WORK_MOBILE:
                                workMobileList.add(CommunicationUtils.normalizePhoneNumber(number));
                                if(!phones.containsKey(Contact.WORK_MOBILE)) phones
                                        .put(Contact.WORK_MOBILE, workMobileList);
                                break;
                            case Phone.TYPE_WORK_PAGER:
                                workPagerList.add(CommunicationUtils.normalizePhoneNumber(number));
                                if(!phones.containsKey(Contact.WORK_PAGER)) phones
                                        .put(Contact.WORK_PAGER, workPagerList);
                                break;
                        }
                    }
                    phoneCur.close();
                    ...
                    while (emailCur.moveToNext()) {
                        ...
                        switch(emailType){
                            case Email.TYPE_HOME:
                                ...
                                if(!emails.containsKey(Contact.HOME_EMAIL)) emails
                                        .put(Contact.HOME_EMAIL,homeEmailList);
                                break;
                            case Email.TYPE_MOBILE:
                                mobileEmailList.add(email);
                                if(!emails.containsKey(Contact.MOBILE_EMAIL)) emails
                                        .put(Contact.MOBILE_EMAIL,mobileEmailList);
                                break;
                            case Email.TYPE_OTHER:
                                otherEmailList.add(email);
                                if(!emails.containsKey(Contact.OTHER_EMAIL)) emails
                                        .put(Contact.OTHER_EMAIL,otherEmailList);
                                break;
                            case Email.TYPE_WORK:
                                workEmailList.add(email);
                                if(!emails.containsKey(Contact.WORK_EMAIL)) emails
                                        .put(Contact.WORK_EMAIL,workEmailList);
                                break;
                        }
                    }
                    emailCur.close();
